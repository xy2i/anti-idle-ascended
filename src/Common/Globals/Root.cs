...
public class Root
{
    ...
    public void gainEXP(double amount, double sauce)
    {
        ...
        if (!isNaN(amount) && amount > 0 && amount != Infinity)
        {
            ...
            {
                var boostToGain = 0.05 + amount / Math.min(_root.requiredExp, 810000000) / Math.pow(_root.save.boost, 1.11) * 35000;
            ...
            if (_root.cursoridle < 30 && _root.save.showGain == true && sauce != 0 && multiplier > 0)
            {
                ...
                _root.mainAntiLag += 1;
                _root.popContain.attachMovie("scorePopMain", "scorePopMain" + _root.mainSummonCount, _root.mainAntiLag, new { _x = 480, _y = 60, what = "+" + _root.withComma(Math.floor(amount * multiplier)), whatColor = 10092288 });
            }
        }
    }
    ...
    public void gainCoin(double amount, double sauce)
    {
        ...
        if (!isNaN(amount) && amount > 0 && amount != Infinity && _root.save.coinOvercap < 300)
        {
            if (_root.save.level < 9001 && _root.save.featureBoostAuto == true)
            {
                var boostToGain = 0.01 + amount / Math.min(_root.requiredExp, 810000000) / Math.pow(_root.save.boost, 1.11) * 7000;
                if (_root.save.level < 125)
                {
                    boostToGain = boostToGain * _root.save.level / 125;
                }
            ...
            if (_root.cursoridle < 30 && _root.save.showGain == true && sauce != 22 && sauce != 0 && sauce != 39)
            {
                _root.mainSummonCount += 1;
                _root.mainAntiLag += 1;
                _root.popContain.attachMovie("scorePopMain", "scorePopMain" + _root.mainSummonCount, _root.mainAntiLag, new { _x = 80, _y = 20, what = "+" + _root.withComma(Math.floor(amount * multiplier)), whatColor = 16776960 });
            }
        }
    ...
    public void gainGreenCoin(double amount)
    {
        if (!isNaN(amount) && amount > 0 && amount != Infinity && _root.save.greenCoinOvercap < 300)
        {
            ...
            if (_root.cursoridle < 30 && _root.save.showGain == true)
            {
                _root.mainSummonCount += 1;
                _root.mainAntiLag += 1;
                _root.popContain.attachMovie("scorePopMain", "scorePopMain" + _root.mainSummonCount, _root.mainAntiLag, new { _x = 10, _y = 60, what = "+" + _root.withComma(Math.floor(amount * multiplier)), whatColor = 65280 });
            }
        }
    ...
    public void gainBlueCoin(double amount)
    {
        if (!isNaN(amount) && amount > 0 && amount != Infinity && _root.save.blueCoinOvercap < 300)
        {
            ...
            if (_root.cursoridle < 30 && _root.save.showGain == true)
            {
                _root.mainSummonCount += 1;
                _root.mainAntiLag += 1;
                _root.popContain.attachMovie("scorePopMain", "scorePopMain" + _root.mainSummonCount, _root.mainAntiLag, new { _x = 100, _y = 60, what = "+" + _root.withComma(Math.floor(amount * multiplier)), whatColor = 39423 });
            }
        }
